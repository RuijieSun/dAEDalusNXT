%% generate required paths for program execution
addpath(genpath('..\..\..\'));


%% load XML File
myAC=class_aircraft('part8-simple787.xml',1);

%% grid settings

% set aerodynamic panel size
myAC.grid_settings.x_max_grid_size=myAC.reference.c_ref/8;
myAC.grid_settings.y_max_grid_size=myAC.reference.b_ref/20;
myAC.grid_settings.aerodynamic_fuselage=0;

% set  structural grid size
myAC.grid_settings.dy_max_struct_grid=myAC.reference.b_ref/30;

%% grid computation
myAC=myAC.compute_grid();

%% create Structural Model
%options
structOpts=class_wingstructure_solver_settings;
%create
[myAC,myStr]=create_structural_model(myAC, structOpts);
myStr=myStr.f_solve_free_modes;

%% Init Coupling
myAC=myAC.compute_force_interpolation_matrix(myStr);

%% fligth state definition
refState=critical_ref_state(myAC,0.5,0); %Mach Number and Altitude


%% aerostructural layout
%define critical state
criticalState=critical_g_maneuver_state(refState,2.5);
aeSolverSettings=class_aeroelastic_solver_settings;
[myAC,myStr,myVLM]= structural_sizing_loop(myAC,myStr,criticalState,myAC.weights,aeSolverSettings);


%% create aeSSM
%aircraft needs UVLM grid information (wake)
myAC.grid_settings.wake=2;
myAC=myAC.compute_grid;

% initialize aeSSM
aeSSMSettings=AeSSMSettings;
aeSSMSettings.modes=[7:16]; % specify elastic modes
aeSSMSettings.wakeLengthFactor=0.1; %performance improvement, better set to >1
myAeSSM=AeSSM(myAC,myStr,refState.aerodynamic_state, aeSSMSettings);

%% run gust analysis
myAeSSM=myAeSSM.runGustAnalysis(refState,1, 5);

% plot result
figure; plot(myAeSSM.gustData.timeVec,    squeeze(myAeSSM.gustData.r(:,3,:)))
xlabel('time [s]')
ylabel('vertical position [m]')

figure; plot(myAeSSM.gustData.timeVec,    squeeze(myAeSSM.gustData.modes(:,1,:)))
xlabel('time [s]')
ylabel('modal deflection [m]')

% write results to tecplot
modalHistory=[zeros(size(myAeSSM.gustData.modes(1:5:end,:,5),1),6)    myAeSSM.gustData.modes(1:5:end,:,1)];
myAC.write_modal_time_history_in_paraview(myStr,modalHistory,'gustAnim','length1')
modalHistory=[zeros(size(myAeSSM.gustData.modes(1:5:end,:,5),1),6)    myAeSSM.gustData.modes(1:5:end,:,3)];
myAC.write_modal_time_history_in_paraview(myStr,modalHistory,'gustAnim','length3')
modalHistory=[zeros(size(myAeSSM.gustData.modes(1:5:end,:,5),1),6)    myAeSSM.gustData.modes(1:5:end,:,5)];
myAC.write_modal_time_history_in_paraview(myStr,modalHistory,'gustAnim','length5')


%% run flutter analysis
VloopVec=[100:25:550];
PlotFlag=1;
figure;
myAeSSM=myAeSSM.runFlutterAnalysis(VloopVec,PlotFlag);

figure; 
 plot(myAeSSM.flutterTable.speedVec,-cos(angle(myAeSSM.flutterTable.eigVal)))
 xlabel('Speed')
 ylabel('Damping')

figure; 
 plot(myAeSSM.flutterTable.speedVec,abs((myAeSSM.flutterTable.eigVal))/2/pi)
 xlabel('Speed')
 ylabel('Frequency [Hz]')


