...
%
function MeanAxisPartitioned=simulate_free_flying_mean_axis_partitioned_w_coefficients(aircraft,aircraft_structure,n_modes,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
Mdiag=aircraft_structure.modeshapes'*aircraft_structure.Mff*aircraft_structure.modeshapes;
...
for i=1:length(t_vec)
   ...
   % XHALE_aero=XHALE_aero.update_p_ref(-(M_BI*mean_axis_origin)');
    XHALE_aero=XHALE_aero.solve_time_domain_aerodynamics(aircraft,[([-1 0 0;0 1 0; 0 0 -1]*MeanAxisPartitioned.Xe(i,:)')' MeanAxisPartitioned.Euler(i,:)],MeanAxisPartitioned.V(i),MeanAxisPartitioned.Alpha(i),MeanAxisPartitioned.Beta(i),...
        [-1 0 0;0 1 0; 0 0 -1]*MeanAxisPartitioned.pqr(i,:)',rho_air,i,['results/' aircraft.name '/' maneuver_name '/movie/mean_axis_partitioned_wcoeff']);
    aircraft=aircraft.compute_beam_forces(XHALE_aero.F_body,aircraft_structure);  
    ...
    structural_acc(1:3)=gravity'+structural_acc(1:3)*0;
    ...
    F=[XHALE_aero.qinf*XHALE_aero.CX(i)*XHALE_aero.reference.S_ref+gravity(1)*Mass(1,1);
    ...
    MeanAxisPartitioned.cF(i,:)=F;
    MeanAxisPartitioned.cM(i,:)=M;
...
MeanAxisPartitioned.Time=t_vec;
save(['results/' aircraft.name '/' maneuver_name '/Simulation_Mean_Axis_Partitioned_w_Coefficients'],'MeanAxisPartitioned','MeanAxisPartitioned');
end