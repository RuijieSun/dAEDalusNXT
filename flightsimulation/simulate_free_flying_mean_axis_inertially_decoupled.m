...
%
function MeanAxis=simulate_free_flying_mean_axis_inertially_decoupled(aircraft,aircraft_structure,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
%[aircraft,aircraft_structure,steady_trim_aero,state ] = flight_state_loop(aircraft,aircraft_structure,trim_state,aeroelastic_solver_settings);
...
for i=1:length(t_vec)
    ...
    [M_tot_mean,K_tot_mean,F_acc_mean]=compute_mean_axis_matrices_inertial_dcpl(A_Bar,I_Hat,b,b_Hat_Skew,omega_Hat_Skew,aircraft_structure.Mff,aircraft_structure.Kff,xdot_now);
    ...
    aircraft_aero=aircraft_aero.solve_time_domain_aerodynamics(aircraft,[([-1 0 0;0 1 0; 0 0 -1]*MeanAxis.Xe(i,:)')' MeanAxis.Euler(i,:)],MeanAxis.V(i),MeanAxis.Alpha(i),MeanAxis.Beta(i),...
        [-1 0 0;0 1 0; 0 0 -1]*MeanAxis.pqr(i,:)',rho_air,i,['results/' aircraft.name '/' maneuver_name '/movie/mean_axis_decoupled']);
...
MeanAxisID=MeanAxis;
save(['results/' aircraft.name '/' maneuver_name '/Simulation_Mean_AxisID'],'MeanAxisID','MeanAxisID');
end