...
%
function [aircraft_structure]=f_compute_loads_elastic(aircraft,aircraft_structure,state,aeroelastic_solver_settings)
...
for state_ctr=1:length(state)
    ...
    uvlm_flag=0; % no uvlm trimming required
    [aircraft,aircraft_structure,steady_trim_aero,trimmed_state,~] = trim_elastic_aircraft(aircraft,aircraft_structure,state(state_ctr),uvlm_flag); %
    % transform forces from aero to structural grid
    aircraft=aircraft.compute_beam_forces(steady_trim_aero.F_body,aircraft_structure);
    
    if aeroelastic_solver_settings.CG_accelerations==1
        aircraft=aircraft.compute_acceleration(steady_trim_aero,aircraft_structure,state(state_ctr));
        %next line is wrong
        aircraft.acc=[0 0 0 0 0 0];
        aircraft_structure=aircraft_structure.f_set_acceleration(-aircraft.acc,steady_trim_aero.reference.p_ref);
    end
